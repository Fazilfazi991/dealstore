...
Record every plausible source candidate in `data/source-research.csv`, including rejected listings and the exact identity conflict. `Accepted` is permitted only when the URL is the approved canonical source; search results and similar garments are not verification.
