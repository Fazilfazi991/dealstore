...
export async function createStripeCheckout(order:CreatedOrder,checkout:CheckoutDetails,items:CheckoutItem[]){
  ...
  const form=new URLSearchParams({mode:"payment",success_url:`${origin}/order-success?order=${encodeURIComponent(order.order_number)}`,cancel_url:`${origin}/checkout?payment=cancelled`,"metadata[order_number]":order.order_number});if(checkout.email)form.set("customer_email",checkout.email);
